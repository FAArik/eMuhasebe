...
export class InvoicesComponent {
    ...
    if (form.valid) {
      this.http.post<string>("Invoices/DeleteById", { id: this.updateModel.id }, (res) => {
        this.http.post<string>("Invoices/Create", this.updateModel, (res) => {
          this.swal.callToast(res, "info");
          this.updateModel = new InvoiceModel();
  ...
  addDetail() {
    const detail: InvoiceDetailModel = {
      ...
      id: "",
      invoiceId: "",
      product: this.products.find(x => x.id == this.createModel.productId) ?? new ProductModel()
    };
  ...
  removeDetail(index: number) {
    this.createModel.details = this.createModel.details.splice(index, 1);
  }
  addDetailForUpdate() {
    const detail: InvoiceDetailModel = {
      ...
      id: "",
      invoiceId: "",
      product: this.products.find(x => x.id == this.updateModel.productId) ?? new ProductModel()
    };
  ...
  removeDetailForUpdate(index: number) {
