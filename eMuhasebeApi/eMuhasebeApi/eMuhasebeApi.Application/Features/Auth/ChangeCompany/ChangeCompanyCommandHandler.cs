...
sealed class ChangeCompanyCommandHandler(ICompanyUserRepository companyUserRepository, UserManager<AppUser> userManager, IHttpContextAccessor httpContextAccessor,IJwtProvider jwtProvider) : IRequestHandler<ChangeCompanyCommand, Result<LoginCommandResponse>>
{
    public async Task<Result<LoginCommandResponse>> Handle(ChangeCompanyCommand request, CancellationToken cancellationToken)
    {
        ...
        List<CompanyUser> companyUsers = await companyUserRepository.Where(x => x.AppUserId == appuser.Id).Include(x => x.Company).ToListAsync(cancellationToken);
