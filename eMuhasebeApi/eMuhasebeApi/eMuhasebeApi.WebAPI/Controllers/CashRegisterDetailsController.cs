...
public sealed class CashRegisterDetailsController : ApiController
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreateCashRegisterDetailCommand command,
    ...
    [HttpPost]
    public async Task<IActionResult> Update(UpdateCashRegisterDetailCommand command,
        CancellationToken cancellationToken)
    {
        var response = await _mediator.Send(command, cancellationToken);
        return StatusCode(response.StatusCode, response);
    ...
    [HttpPost]
    public async Task<IActionResult> DeleteById(DeleteCashRegisterDetailByIdCommand command,
        CancellationToken cancellationToken)
    {
        var response = await _mediator.Send(command, cancellationToken);
        return StatusCode(response.StatusCode, response);
